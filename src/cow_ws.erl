...
-type close_code() :: undefined | 1000..1003 | 1006..1011 | 3000..4999.
...
-spec negotiate_permessage_deflate(list(), #extensions{}, #extensionOpts{}) ->
    ignore | {ok, list(), #extensions{}}.
negotiate_permessage_deflate(_, #extensions{deflate = Deflate, deflate_window_bits = WindowBits}, _)
	when Deflate =/= undefined orelse WindowBits =/= undefined ->
	ignore;
negotiate_permessage_deflate(Params, _Extensions, #extensionOpts{client_takeover = OptsClientTakeOver,
	client_max_window = OptsClientWindow, server_takeover = OptsServerTakeOver,
	server_max_window = OptsServerWindow, compression_threshold = Threshold}) ->
	case lists:usort(Params) of
		%% Ignore if multiple parameters with the same name.
		Params2 when length(Params) =/= length(Params2) ->
			ignore;
		Params2 ->
			case parse_request_permessage_deflate_params(Params2, OptsClientWindow, OptsClientTakeOver,
				OptsServerWindow, OptsServerTakeOver, []) of
				ignore ->
					ignore;
				{ClientWindowBits, ClientTakeOver, ServerWindowBits, ServerTakeOver, RespParams} ->
					{Inflate, Deflate} = init_permessage_deflate(ClientWindowBits, ClientTakeOver, ServerWindowBits, ServerTakeOver),
...
%% If our situation regarding takeover is still 'negotiable' after parsing the request,
%% then default to takeover. Otherwise check the response has the forced params.
...
parse_request_permessage_deflate_params([<<"client_no_context_takeover">>|Tail], CB, _STO, SB, STO, RespParams) ->
...
parse_request_permessage_deflate_params([<<"server_no_context_takeover">>|Tail], CB, CTO, SB, _STO, RespParams) ->
...
init_permessage_deflate(InflateWindowBits, _ClientTakeOver, DeflateWindowBits, ServerTakeOver) ->
    InflateStream = zlib:open(),
    ok = zlib:inflateInit(InflateStream, -InflateWindowBits),
    DeflateStream = case ServerTakeOver of
                        takeover ->
                            Deflate = zlib:open(),
...
-spec negotiate_x_webkit_deflate_frame(list(), #extensions{}, integer()) ->
    ignore | {ok, binary(), #extensions{}}.
negotiate_x_webkit_deflate_frame(_, #extensions{deflate = Deflate, deflate_window_bits = WindowBits}, _)
	when Deflate =/= undefined orelse WindowBits =/= undefined ->
	ignore;
negotiate_x_webkit_deflate_frame(_Params, _Extensions, Threshold) ->
	...
	{Inflate, Deflate} = init_permessage_deflate(15, takeover, 15, takeover),
...
parse_header(<< _:1, Rsv:3, _/bits >>, #extensions{deflate=Deflate, deflate_window_bits = DeflatedWindowBits, deflate_takeover = Takeover}, _)
    when Deflate =:= undefined, DeflatedWindowBits =:= undefined, Takeover =:= undefined, Rsv =/= 0 -> error;
%% Last 2 RSV bits MUST be 0 if deflate-frame extension is used.
parse_header(<< _:2, 1:1, _/bits >>, #extensions{deflate=Deflate}, _) when Deflate =/= undefined -> error;
parse_header(<< _:3, 1:1, _/bits >>, #extensions{deflate=Deflate}, _) when Deflate =/= undefined -> error;
...
parse_payload(Data, _, Utf8State, _, _, 0, {fin, _, << 1:1, 0:2 >>},
		#extensions{inflate=Inflate, inflate_takeover=TakeOver}, _) ->
...
parse_payload(Data, MaskKey, Utf8State, ParsedLen, Type, Len, FragState,
		#extensions{inflate = Inflate, inflate_takeover = TakeOver}, << 1:1, 0:2 >>) when Type =:= text; Type =:= binary ->
	{Data2, Rest, Eof} = split_payload(Data, Len),
	Payload = inflate_frame(unmask(Data2, MaskKey, ParsedLen), Inflate, TakeOver, FragState, Eof),
	validate_payload(Payload, Rest, Utf8State, ParsedLen, Type, FragState, Eof);
%% Empty frame.
...
%% IOList Thresholds
frame({text, Payload}, #extensions{deflate_takeover = TakeOver,
        compression_threshold = Threshold} = Extensions) when is_list(Payload) ->
    case iolist_size(Payload) >= Threshold of
        true ->
            Payload2 = deflate_frame(Payload, Extensions, TakeOver),
...
frame({binary, Payload}, #extensions{deflate_takeover = TakeOver,
        compression_threshold = Threshold} = Extensions) when is_list(Payload) ->
    case iolist_size(Payload) >= Threshold of
        true ->
            Payload2 = deflate_frame(Payload, Extensions, TakeOver),
...
%% Binary Thresholds
frame({text, Payload}, #extensions{ deflate_takeover = TakeOver,
        compression_threshold = Threshold} = Extensions) when byte_size(Payload) >= Threshold ->
	Payload2 = deflate_frame(Payload, Extensions, TakeOver),
	Len = payload_length(Payload2),
	[<< 1:1, 1:1, 0:2, 1:4, 0:1, Len/bits >>, Payload2];
frame({binary, Payload}, #extensions{deflate_takeover = TakeOver,
...
%% Data frames, deflate-frame extension.
%% IOList Thresholds
masked_frame({text, Payload}, #extensions{deflate_takeover = TakeOver,
	compression_threshold = Threshold} = Extensions) when is_list(Payload) ->
    MaskKeyBin = << MaskKey:32 >> = crypto:rand_bytes(4),
    case iolist_size(Payload) >= Threshold of
        true ->
            Payload2 = mask(deflate_frame(Payload, Extensions, TakeOver), MaskKey, <<>>),
            Len = payload_length(Payload2),
...
masked_frame({binary, Payload}, #extensions{deflate_takeover = TakeOver,
        compression_threshold = Threshold} = Extensions) when is_list(Payload) ->
    MaskKeyBin = << MaskKey:32 >> = crypto:rand_bytes(4),
    case iolist_size(Payload) >= Threshold of
        true ->
...
deflate_frame(Payload, #extensions{ deflate = DeflateStream }, takeover) ->
	DeflatedPayload = iolist_to_binary(zlib:deflate(DeflateStream, Payload, sync)),
	Len = byte_size(DeflatedPayload) - 4,
	case DeflatedPayload of
		<< Body:Len/binary, 0:8, 0:8, 255:8, 255:8 >> -> Body;
		_ -> DeflatedPayload
	end;
deflate_frame(Payload, #extensions{ deflate_window_bits = DeflateWindowBits}, no_takeover) ->
	DeflateStream = zlib:open(),
	DeflateWindowBits2 = case DeflateWindowBits of
							 8 -> 9;
							 _ -> DeflateWindowBits
						 end,
	ok = zlib:deflateInit(DeflateStream, best_compression, deflated, -DeflateWindowBits2, 8, default),
	DeflatedPayload = iolist_to_binary(zlib:deflate(DeflateStream, Payload, sync)),
	zlib:deflate(DeflateStream, [], finish),
	ok = zlib:deflateEnd(DeflateStream),
	zlib:close(DeflateStream),
	Len = byte_size(DeflatedPayload) - 4,
	case DeflatedPayload of
		<< Body:Len/binary, 0:8, 0:8, 255:8, 255:8 >> -> Body;
		_ -> DeflatedPayload
